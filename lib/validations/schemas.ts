...
export const umkmSchema = z.object({
  // Name/owner/whatsapp are optional so a UMKM can be added with only a
  // category (e.g. "we know a Kuliner business exists but haven't
  // collected its details yet"). When provided, they must still meet the
  // usual minimums.
  name: z.string().min(3, "Nama usaha minimal 3 karakter").max(150).optional().or(z.literal("")),
